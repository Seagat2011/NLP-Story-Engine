...
var g_token = {// GUID (Globally Unique ID)
...
var clause_element = {} // clause_element {}
...
var compund_subject_element = {} // complex_sentence_element {}
...
var verb_grammar = function(){                  
    ...
    (b[args[0]][args[1]]&&args.length<3) ||
    (b[args[0]][args[1]][args[2]]&&args.length<4) ||    
    (c[args[0]][args[1]]&&arg.length<3)
    ){    
        status = true                           
    }                                           
    return status                               
...
var predicate_grammar = function (){   
    ...
    (c[k[0]][k[1]][k[2]]) ||          
    (e[k[0]][k[1]][k[2]]) ||              
    (b[k[0]][k[1]]&&k.length<3) ||              
    (d[k[0]][k[1]]&&k.length<3) ||              
    (a[k[0]]&&k.length<2)
    ){
        status = true
    }    
...
function loader() {
    srcCode.value = "sequence_blocks=4 iterative_depth=0 character_attributes=0"
    return
} // function loader ()
...
function translatorTool() {    
   ...
   function Word (){    
        var nums        = srcCode.value.match(/d+/)
        var profiles    = srcCode.value.match( /'(w+)'/)
        var character_attributes = {
            remark: {
                location: {
                    0:["in the mountains ","zanzibar  Such cold and harsh mountains having very little vegetation and sharp cragged rock  Great place if you're hungry for death  Which I was not  I would hurry to get out of this place\n\n"],
                    1:["on the sands ", "golgotha  I could sense my luck running low  This is much worse than luck running out because even though the suffering is just as but unfortunately you will always live through it\n\n"],
                    2:["in the valley ", "swat  Not always an easy place to start  But its better than dead  I would take inventory of my supplies\n\n"],
                    3:["beside the lake ", "zinj  A most beautiful lake this time of year  I can only imagine how the soldiers anticipate this well-welcomed break  If an enemy comes there will be over a minute to ready ourselves and to strap on our gear  I look forward to these straightforward moments\n\n"],
                    4:["on the lake ", "torr  It feels cool outside  I wonder how the others are doing in the colder climates  No doubt they will forge through  But I wouldn't want to imagine it\n\n"],
                    5:["on the hills ", "azur  Great lake  Lots of sun  No food\n\n"],
                    6:["in the land ", "the dark Forest  Terrible place to start: no food shelter, water, or weapons  We would have to make everything from the petrified wood around us  One of the few good things about this unwelcoming place\n\n"],
                    7:["on the shores ", "the biden river  Plenty of food here  Unfortunately its also one of the few salt-water rivers in our land  Even the fish taste like salt  We have food but we wont eat\n\n"],
                    8:["in the shade ", "the breadwoods  I wondered how my soldiers would welcome this place as much as I do  I've warmed very much to this place  Which is why I almost always keep my weapon a staggering two paces from me  Here I feel like I can relax\n\n"],
                    parent_index: 8,
                    child_index:-1,
                }, // location {}
                procession: {
                    0:["the mountains of zanzibar ", "I would make my way gradually through  Stopping only when necessary  The mountains were dangerous: the z'abel-coor had successfully ambushed an entire team and killed them   I would not want to waste time there  "],
                    1:["the sands of golgotha ", "I would need to plan my trip carefully  There would be very little room for error here  A previous general had ordered one thousand sentinels to these sand-drifts only to see one return  And I would not have the luxury of such well-trained men  "],
                    2:["the valley of swat ", "this would also be no small task  How do I explain the almost certainty of death for a great majority of my men  I reasoned it best just not to tell  "],
                    3:["the lake of zinj ", "I would not have very much time in the locale  It was important to keep moving  I would have to do this without wearing the troops down  "],
                    4:["the lake of torr ", "I would try to cross the lake quietly  The Greys were a nasty still-water entity that thrived on the ignorance of the uninitiated  If I failed here, I would likely have few men to go any further  "],
                    5:["the hills of azur ","I would travel by force  Cutting and hacking my way through the countryside  There was little threat in these lands if we crossed before dark  But if we ran late, then we would not survive  "],
                    6:["the land of the dark Forest ", "I would travel with caution, without the ability to travel in the trees like the birds which was the safest way  We march our way through and we would do it fast  "],
                    7:["the shores of the biden river ", "my men and I would rest as needed  This was one of the few places where if a threat came, It would do so from only one direction: the way in which we came  We would have to watch our progressions carefully  "],
                    8:["the shade of the breadwoods  ", "an effort would be made here to forge new weapons  We would need an arsenal before making our way to the sentinel king  "],
                    parent_index: 8,
                    child_index:-1,                                                
                } // procession {}
            } // remark {}
        ...
        var basic_template = {
            location:["I was ","of "],
            progression:["where ", "And finally onto ",
                        "I would proceed to ",
    ...
    function Paragraph () {    
        this.DENOUEMENT = function (){
            var result = []
            var s = new Sentence()
            result.push(s.DENOUEMENT('+'))
            result.push(s.CLIMAX())
            result.push(s.CRISIS())
            result.push(s.SEQ02())
            result.push(s.PINCH())
            result.push(s.SEQ01())
            result.push(s.BIG_EVENT())
            result.push(s.CATALYST())
            result.push(s.BIG_OPENING())
            return 'SEN DENOUEMENT' + result.reverse().join ('\n\n')                
        }
        this.CLIMAX = function (){
            var result = []
            var s = new Sentence()
            ...
            result.push(s.CATALYST())
            result.push(s.BIG_OPENING())
            return 'SEN CLIMAX' + result.reverse().join ('\n\n')                
        }
        this.CRISIS = function (){
            var result = []
            var s = new Sentence()
            ...
            result.push(s.CATALYST())
            result.push(s.BIG_OPENING())
            return 'SEN CRISIS' + result.reverse().join ('\n\n')             
        }
        this.SEQ02 = function (){
            var result = []
            var s = new Sentence()
            ...
            result.push(s.CATALYST())
            result.push(s.BIG_OPENING())
            return 'SEN SEQ02' + result.reverse().join ('\n\n')    
        }
        this.PINCH = function (){
            var result = []
            var s = new Sentence()
            ...
            result.push(s.CATALYST())
            result.push(s.BIG_OPENING())
            return 'SEN PINCH' + result.reverse().join ('\n\n')    
        }
        this.SEQ01 = function (){
            var result = []
            var s = new Sentence()
            ...
            result.push(s.CATALYST())
            result.push(s.BIG_OPENING())
            return 'SEN SEQ01' + result.reverse().join ('\n\n')   
        }
        this.BIG_EVENT = function (){
            var result = []
            var s = new Sentence()
            ...
            result.push(s.CATALYST())
            result.push(s.BIG_OPENING())
            return 'SEN BIG_EVENT' + result.reverse().join ('\n\n')    
        }
        this.CATALYST = function (){
            var result = []
            var s = new Sentence()
            ...
            result.push(s.CATALYST())
            result.push(s.BIG_OPENING())
            return 'SEN CATALYST' + result.reverse().join ('\n\n')   
        }
        this.BIG_OPENING = function (){
            var result = []
            var s = new Sentence()
            ...
            result.push(s.CATALYST())
            result.push(s.BIG_OPENING())
            return 'SEN BIG_OPENING' + result.reverse().join ('\n\n')   
        }                
    }
    Paragraph.prototype = new Object ()
    ...
    function Sequence () {
        this.DENOUEMENT = function (){
            ...
            result.push(s.CATALYST())
            result.push(s.BIG_OPENING())
            return 'P DENOUEMENT' + result.reverse().join ('\n\n')              
        }
        this.CLIMAX = function (){
            var result = []
            var s = new Paragraph()
            ...
            result.push(s.CATALYST())
            result.push(s.BIG_OPENING())
            return 'P CLIMAX' + result.reverse().join ('\n\n')            
        }
        this.CRISIS = function (){
            var result = []
            var s = new Paragraph()
            ...
            result.push(s.CATALYST())
            result.push(s.BIG_OPENING())
            return 'P CRISIS' + result.reverse().join ('\n\n')               
        }
        this.SEQ02 = function (){
            var result = []
            var s = new Paragraph()
            ...
            result.push(s.CATALYST())
            result.push(s.BIG_OPENING())
            return 'P SEQ02' + result.reverse().join ('\n\n')    
        }
        this.PINCH = function (){
            var result = []
            var s = new Paragraph()
            ...
            result.push(s.CATALYST())
            result.push(s.BIG_OPENING())
            return 'P PINCH' + result.reverse().join ('\n\n')    
        }
        this.SEQ01 = function (){
            var result = []
            var s = new Paragraph()
            ...
            result.push(s.CATALYST())
            result.push(s.BIG_OPENING())
            return 'P SEQ01' + result.reverse().join ('\n\n')   
        }
        this.BIG_EVENT = function (){
            var result = []
            var s = new Paragraph()
            ...
            result.push(s.CATALYST())
            result.push(s.BIG_OPENING())
            return 'P BIG_EVENT' + result.reverse().join ('\n\n')   
        }
        this.CATALYST = function (){
            var result = []
            var s = new Paragraph()
            ...
            result.push(s.CATALYST())
            result.push(s.BIG_OPENING())
            return 'P CATALYST' + result.reverse().join ('\n\n')   
        }
        this.BIG_OPENING = function (){
            var result = []
            var s = new Paragraph()
            ...
            result.push(s.CATALYST())
            result.push(s.BIG_OPENING())
            return 'P BIG_OPENING' + result.reverse().join ('\n\n')   
        }                
    }
    Sequence.prototype = new Object ()
    ...
    function Story() {    
        this.DENOUEMENT = function (){
            ...
            result.push(s.CATALYST())
            result.push(s.BIG_OPENING())
            return 'SEQ DENOUEMENT' + result.reverse().join ('\n\n')  
        }
        this.CLIMAX = function (){
            var result = []
            var s = new Sequence()
            ...
            result.push(s.CATALYST())
            result.push(s.BIG_OPENING())
            return 'SEQ CLIMAX' + result.reverse().join ('\n\n')  
        }
        this.CRISIS = function (){
            var result = []
            var s = new Sequence()
        ...
        this.SEQ02 = function (){
            ...
            result.push(s.CATALYST())
            result.push(s.BIG_OPENING())
            return 'SEQ SEQ02' + result.reverse().join ('\n\n')  
        }
        this.PINCH = function (){
            var result = []
            var s = new Sequence()
            ...
            result.push(s.CATALYST())
            result.push(s.BIG_OPENING())
            return 'SEQ PINCH' + result.reverse().join ('\n\n')  
        }
        this.SEQ01 = function (){
            var result = []
            var s = new Sequence()
            ...
            result.push(s.CATALYST())
            result.push(s.BIG_OPENING())
            return 'SEQ SEQ01' + result.reverse().join ('\n\n')  
        }
        this.BIG_EVENT = function (){
            var result = []
            var s = new Sequence()
            ...
            result.push(s.CATALYST())
            result.push(s.BIG_OPENING())
            return 'SEQ BIG_EVENT' + result.reverse().join ('\n\n')  
        }
        this.CATALYST = function (){
            var result = []
            var s = new Sequence()
            ...
            result.push(s.CATALYST())
            result.push(s.BIG_OPENING())
            return 'SEQ CATALYST' + result.reverse().join ('\n\n') 
        }
        this.BIG_OPENING = function (){
            var result = []
            var s = new Sequence()
            ...
            result.push(s.CATALYST())
            result.push(s.BIG_OPENING())
            return 'SEQ BIG_OPENING' + result.reverse().join ('\n\n')  
        }        
    } // Story () constructor
